...
// Map old static html paths to Express routes
function mapHref(href) {
  if (!href || typeof href !== "string") return "#";
  return href
    .replace(/\/index\.html$/i, "/")
    .replace(/\/about\.html$/i, "/about")
    .replace(/\/project\.html$/i, "/projects")
    .replace(/\/projects\.html$/i, "/projects")
    .replace(/\/blog\.html$/i, "/blog");
}

// Shared data loader (with href normalization)
async function loadShared() {
  const [navLinksRaw, socialLinks] = await Promise.all([
    readJson("navLinks.json").then((d) => d.navLinks || d.navLink || []),
    readJson("socialLinks.json").then((d) => d.socialLinks || []),
  ]);
  const navLinks = navLinksRaw.map((l) => ({ ...l, href: mapHref(l.href) }));
...
app.get("/api/nav-links", async (req, res) => {
  ...
  res.json({ navLinks: list.map((l) => ({ ...l, href: mapHref(l.href) })) });
});
